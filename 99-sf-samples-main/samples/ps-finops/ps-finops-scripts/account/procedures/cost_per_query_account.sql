...
CREATE OR REPLACE PROCEDURE COST_PER_QUERY_ACCOUNT()
 RETURNS VARCHAR
 LANGUAGE SQL
...
Revision:
2024/03/25 CL Bug fix for QAS credits. Very rare occurrence of query_acceleration_history spanning multiple time periods and nothing to match in query_history.
2024/05/13 CL Revised compute job exclusion filters in query_history and wmh
2025/04/03 CL Added filter to exclude queries on Snowflake Compute Service.
2025/05/20 CL Added CALL statements and incorporated query_load_percent into warehouse_weight
******/
...
INSERT INTO SF_CREDITS_BY_QUERY (
    QUERY_ID,
    START_SLICE,
    ADJUSTED_START_TIME,
    START_TIME,
...
WITH SF_WAREHOUSE_METERING_HISTORY AS(
    ...
    FROM
        ...
        LEFT JOIN SNOWFLAKE.ACCOUNT_USAGE.QUERY_ACCELERATION_HISTORY Q ON WMH.START_TIME = Q.START_TIME
...
, QUERIES_BY_TIME_SLICE AS (
    SELECT
        ...
        RATIO_TO_REPORT(DERIVED_ELAPSED_TIME_MS::NUMBER(38, 0)*QH.WAREHOUSE_WEIGHT) OVER (PARTITION BY QH.WAREHOUSE_ID, DD.START_SLICE) AS ELAPSED_TIME_RATIO,
		RATIO_TO_REPORT(QAS_BYTES_USED) OVER (PARTITION BY QH.WAREHOUSE_ID, DD.START_SLICE) AS QAS_RATIO
    FROM
